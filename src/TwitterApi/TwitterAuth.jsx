  ...
  const getImageUrl = (eachTweet) => {
    ...
    if(eachTweet.hasOwnProperty("attachments")){
      ...
      const getImageArray = _.map(mediaKeys,each => {
       return includes.map(eachIncludes => _.get(eachIncludes,"media_key") === each ? {type: _.get(eachIncludes,"type",""),url: _.get(eachIncludes,"url")} : null  )
      })
        ...
        _.map(data, (each) => {
          ...
          <div>
            ...
            <h2 key={_.get(each,'id','')}>
              ...
              {
               _.size(getImageUrl(each)) ? _.map(getImageUrl(each), eachImageUrl => {
                  return <div >
                    {
                      eachImageUrl ? eachImageUrl.type === "photo" ?
                      <img className="mx-auto"  src={eachImageUrl.url} alt="tweetimg" />
                      :
                      null
                      :
                      null
